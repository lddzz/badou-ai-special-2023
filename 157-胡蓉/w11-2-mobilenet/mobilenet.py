...
def MobileNet(input_shape=[224,224,3], dropout=1e-3, classes=1000):
    ...
    # 56,56,128 -> 28,28,256
    x = _depthwise_conv_block(x, 256,   strides=(2, 2), block_id=4)
    
    # 28,28,256 -> 28,28,256
    x = _depthwise_conv_block(x, 256,  block_id=5)

    # 28,28,256 -> 14,14,512
    x = _depthwise_conv_block(x, 512, strides=(2, 2), block_id=6)
    ...
    # 14,14,512 -> 7,7,1024
    x = _depthwise_conv_block(x, 1024, strides=(2, 2), block_id=12)
...
def _depthwise_conv_block(inputs, pointwise_conv_filters, strides=(1, 1), block_id=1):

    x = DepthwiseConv2D((3, 3), padding='same', depth_multiplier=1, strides=strides, use_bias=False,
