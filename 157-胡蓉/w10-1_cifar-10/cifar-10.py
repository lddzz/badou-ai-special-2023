...
#   2.使用函数tf.nn.l2_loss()计算权重均方误差L2 loss
...
def variable_with_weight_loss(shape, stddev, w1):
    var = tf.Variable(tf.truncated_normal(shape, stddev=stddev))  # 和一般的正态分布的产生随机数据相比，这个函数产生的随机数与均值的差距不会超过两倍的标准差
    if w1 is not None:
        weights_loss = tf.multiply(tf.nn.l2_loss(var), w1, name="weights_loss")
        tf.add_to_collection("losses", weights_loss)  # 将weights_loss放入名为"losses"的集合中
    return var


# 使用Cifar10_data.py文件里面已经定义好的文件序列读取函数读取训练数据文件和测试数据从文件.
# 其中训练数据文件进行数据增强处理，测试数据文件不进行数据增强处理
images_train, labels_train = Cifar10_data.inputs(data_dir=data_dir, batch_size=batch_size, distorted=True)  # 进行图片增强处理
images_test, labels_test = Cifar10_data.inputs(data_dir=data_dir, batch_size=batch_size, distorted=None)  # 不进行图片增强处理
...
conv1 = tf.nn.conv2d(x, kernel1, [1, 1, 1, 1], padding="SAME")  # kernel_size=5x5, strides=1
bias1 = tf.Variable(tf.constant(0.0, shape=[64]))
relu1 = tf.nn.relu(tf.nn.bias_add(conv1, bias1))
# ksize : 池化窗口的大小，取一个四维向量，一般是[1, height, width, 1]，因为我们不想在batch和channels上做池化，所以这两个维度设为了1
pool1 = tf.nn.max_pool(relu1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")  # kernel_size=3x3, strides=2
...
pool2 = tf.nn.max_pool(relu2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding="SAME")  # shape=(100,6,6,64)

# 因为要进行全连接层的操作，所以这里使用tf.reshape()函数将pool2输出变成一维向量，并使用get_shape()函数获取扁平化之后的长度
reshape = tf.reshape(pool2, [batch_size, -1])  # 这里面的-1代表将pool2的三维结构拉直为一维结构 shape=(100,2304)
dim = reshape.get_shape()[1].value  # get_shape()[1].value表示获取reshape之后的第二个维度的值 dim=2304

# 建立第一个全连接层
weight1 = variable_with_weight_loss(shape=[dim, 384], stddev=0.04, w1=0.004)  # shape=(ci,co)
...
# sparse_softmax_cross_entropy_with_logits时labels为数字编码，softmax_cross_entropy_with_logits为one-hot编码
cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=result, labels=tf.cast(y_, tf.int64))

weights_with_l2_loss = tf.add_n(tf.get_collection("losses"))  # 取出"losses"集合中的值
...
with tf.Session() as sess:
    ...
    for step in range(max_steps):
        ...
        # image_batch, label_batch = sess.run([images_train[:500], labels_train[:500]])
    ...
    # 计算最终的正确率
    num_batch = math.ceil(num_examples_for_eval / batch_size)  # math.ceil()函数用于向上求整
    true_count = 0
