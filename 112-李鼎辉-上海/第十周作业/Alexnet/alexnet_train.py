...
def main():
    ...
    train_loader = torch.utils.data.DdataLoader(train_dataset,
    ...
    validate_loader = torch.utils.data.DataLoader(validate_dataset,batch_size=batch_size,shuffle=True,
                                                  num_workers=nw)
    print("using {} images for training, {} images for validation.".format(train_nums, val_num))

    net = AlexNet(num_classes=5, init_weight=True)
    net.to(device)
    loss_function = nn.CrossEntropyLoss()
    # pass = list(net.parameters())
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            print('[epoch %d]+train_loss:%.3f val_accuracy:%.3f)'%(epoch+1, running_loss/train_steps
                                                                   ,val_accurate))
