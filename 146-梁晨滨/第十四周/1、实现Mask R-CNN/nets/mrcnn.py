...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # 回归分支：预测每个框的四个参数偏移量(x,y,w,h)
    # x(B, num_rois, cls * 4)         4=(dy, dx, log(dh), log(dw))
    x = TimeDistributed(Dense(num_classes * 4, activation='linear'), name='mrcnn_bbox_fc')(shared)
    # (B, num_rois, cls * 4) ->  (B, num_rois, cls, 4)
...
def get_predict_model(config):
    ...
    # 获得Resnet中的5层特征
    _, C2, C3, C4, C5 = resnet(input_image, stage5=True, train_bn=config.TRAIN_BN)

    # 组成特征金字塔的5层结构
    # P5(H/32,W/32,256) P4(H/16,W/16,256) P3(H/8,W/8,256) P2(H/4,W/4,256)
    ...
    # 各自进行一次256通道的卷积，使P2、P3、P4、P5通道数相同
    ...
    # 由P5获得P6，也用于后续目标框生成P6(H/64,W/64,256)
    ...
    # 作为输出
    model = Model([input_image, input_image_meta, input_anchors], [detections, mrcnn_class, mrcnn_bbox,
