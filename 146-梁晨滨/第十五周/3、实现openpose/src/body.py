...
class Body(object):
    ...
    def __call__(self, image):
        ...
        # 这里需要19个通道去预测19个关键点,38个通道预测19个关键点中每个关键点的2方向
        heatmap_avg = np.zeros((h, w, 19))
        paf_avg = np.zeros((h, w, 38))
        ...
        for m in range(len(multiplier)):
            ...
            image_input = np.transpose(np.float32(imageToTest_padded[:, :, :, np.newaxis]), (3, 2, 0, 1)) / 256 - 0.5
            ...
            # heatmap(关节点坐标图):对输出特征图1整形并去掉padding,还原成输入图像的格式
            ...
            # paf(关键点方向图):对输出特征图2整形并去掉padding,还原成输入图像的格式
        ...
        for k in range(len(mapIdx)):
            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        # 计算距离，首先逐元素相减，然后算平方根 (x^2 + y^2)开方
