...
class Tracker:
    ...
    # 用上一次的结果计算本轮的预测值，之后进入update更新阶段
    def predict(self):
    ...
    def update(self, detections):
        ...
        for index_track, index_detection in matches:
            self.tracks[index_track].update(self.kf, detections[index_detection])
        # 对于未匹配的成功的track，将其标记为丢失
        for index_track in unmatched_tracks:
            self.tracks[index_track].mark_missed()
        # 对于未匹配成功的detection，初始化为新的track
        for index_detection in unmatched_detections:
            self._initiate_track(detections[index_detection])
        self.tracks = [t for t in self.tracks if not t.is_deleted()]

        # 遍历所有tracks，将已经确定的track的特征向量存入metric
        true_targets = [index_tracks.track_id for index_tracks in self.tracks if index_tracks.is_confirmed()]
        features, targets = [], []
        for track in self.tracks:
        ...
        self.metric.partial_fit(
            np.asarray(features), np.asarray(targets), true_targets)
